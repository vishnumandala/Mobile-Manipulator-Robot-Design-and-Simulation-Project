...
def generate_launch_description():
    ...
    pkg_gazebo = get_package_share_directory('bot')
    ...
    call_conveyor_service = ExecuteProcess(
        cmd=['ros2', 'service', 'call', '/CONVEYORPOWER', 'conveyorbelt_msgs/srv/ConveyorBeltControl', '{power: 20.0}'],
        name='call_conveyor_service'
    )
    
    # TimerAction to delay the service call
    delayed_service_call = TimerAction(
        period=2.0,  # Delay in seconds
        actions=[call_conveyor_service]
    )
